...
Short proofs are a \emph{symptom} of good quality code.
...
This is perfectly valid code, but
...
Tactics such as \cverb{apply} and \cverb{refine} isolate intermediate goals that are sufficient to prove the goal.
...
The corresponding informal analogue is arguing that ``to prove what we want, it suffices to show $\ldots$''.
...
\bigskip
It also identifies clear candidates for API lemmas, since each subtree is a separate target.
\end{frame}
...
Alignment with intended goals should be automatically enforced wherever possible.
\end{block}
...
Dedicated agents that provide details, fill in gaps, suggest intermediate goals, state results in a ``formalisation-friendly'' way are immensely valuable.
